//
//  JKSlideMenuController.swift
//  StrateGames
//
//  Created by Jitendra Kumar on 21/05/19.
//  Copyright © 2019 Jitendra Kumar. All rights reserved.
//

import UIKit

@objc public protocol JKSlideMenuControllerDelegate {
...
public struct JKSlideMenuOptions{
    
    public static var leftViewWidth: CGFloat = Platform.isPad ? UIScreen.main.bounds.size.width*0.42: UIScreen.main.bounds.size.width//270.0
//    public static var leftViewWidth: CGFloat = Platform.isPad ? UIScreen.main.bounds.size.width*0.42: UIScreen.main.bounds.size.width*0.72//270.0
    ...
    public static var rightViewWidth: CGFloat = Platform.isPad ? UIScreen.main.bounds.size.width*0.42: UIScreen.main.bounds.size.width*0.72
...
open class JKSlideMenuController: UIViewController ,UIGestureRecognizerDelegate{
    
    public enum JKSlideAction{
        case Open
        case Close
    }
    public enum JKTrackAction {
        case LeftTapOpen
        case LeftTapClose
        case LeftFlickOpen
        case LeftFlickClose
    ...
    struct PanInfo {
        var action: JKSlideAction
        var shouldBounce: Bool
        var velocity: CGFloat
    }
    public weak var delegate: JKSlideMenuControllerDelegate?
    ...
    public var enableleftPanGesture: Bool = false{
        didSet{
            self.view.setNeedsDisplay()
        }
    }
    public var enableleftTapGesture: Bool = false{
        didSet{
            self.view.setNeedsDisplay()
        }
    }
    
    public var enablerightPanGesture: Bool = false{
        didSet{
            self.view.setNeedsDisplay()
        }
    }
    
    public var enablerightTapGesture: Bool = false{
        didSet{
            self.view.setNeedsDisplay()
        }
    }
    ...
    public convenience init(mainViewController: UIViewController, leftMenuViewController: UIViewController?, enableLeftPanGeture:Bool = false,enableLeftTapGeture:Bool = false, rightMenuViewController: UIViewController?,enableRightPanGesture:Bool = false,enableRightTapGesture:Bool = false) {
        self.init(nibName:nil,bundle:nil)
        self.mainViewController = mainViewController
        leftViewController = leftMenuViewController
        enableleftPanGesture = enableLeftPanGeture
        enableleftTapGesture = enableLeftTapGeture
        rightViewController = rightMenuViewController
        enablerightPanGesture = enableRightPanGesture
        enablerightTapGesture = enableRightTapGesture
        initView()
    }
    public convenience init(mainViewController: UIViewController, leftMenuViewController: UIViewController, enableLeftPanGeture:Bool = false,enableLeftTapGeture:Bool = false) {
        self.init(mainViewController: mainViewController, leftMenuViewController: leftMenuViewController, enableLeftPanGeture:enableLeftPanGeture,enableLeftTapGeture: enableLeftTapGeture, rightMenuViewController: nil,enableRightPanGesture: false,enableRightTapGesture:false)
    }
    
    public convenience init(mainViewController: UIViewController, rightMenuViewController: UIViewController,enableRightPanGesture:Bool = false,enableRightTapGesture:Bool = false) {
        self.init(mainViewController: mainViewController, leftMenuViewController: nil, enableLeftPanGeture:false,enableLeftTapGeture: false, rightMenuViewController: rightMenuViewController,enableRightPanGesture: enableRightPanGesture,enableRightTapGesture:enableRightTapGesture)
    }
    ...
    public func initView() {
        ...
        opacityView.backgroundColor = JKSlideMenuOptions.opacityViewBackgroundColor
        ...
        if leftViewController != nil {
            ...
            leftFrame.size.width = JKSlideMenuOptions.leftViewWidth
        ...
        if rightViewController != nil {
            ...
            rightFrame.size.width = JKSlideMenuOptions.rightViewWidth
    ...
    open override func viewWillLayoutSubviews() {
        // topLayoutGuideTo set the various ViewController at this timing that the value is determined
...
//MARK: Change here if not visible; aakarshit
//        setOpenWindowLevel()
        setCloseWindowLevel()
    ...
    public func addLeftGestures() {
        ...
        if leftViewController != nil{
            if enableleftPanGesture == true{
                if leftPanGesture == nil {
                    leftPanGesture = UIPanGestureRecognizer(target: self, action:  #selector(self.handleLeftPanGesture(panGesture:)))
                    leftPanGesture!.delegate = self
                    view.addGestureRecognizer(leftPanGesture!)
                }
                
            }
            if enableleftTapGesture == true{
    ...
    public func addRightGestures() {
        ...
        if rightViewController != nil{
            if enablerightPanGesture == true {
                if rightPanGesture == nil {
                    rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(self.handleRightPanGesture(panGesture:)))
                    rightPanGesture!.delegate = self
                    view.addGestureRecognizer(rightPanGesture!)
                }
            }
            
            if enablerightTapGesture == true{
    ...
    public func track(trackAction: JKTrackAction) {
    ...
    @objc func handleRightPanGesture(panGesture: UIPanGestureRecognizer) {
        ...
        switch panGesture.state {
        case UIGestureRecognizer.State.began:
            if THRightPanState.lastState != .ended &&  THRightPanState.lastState != .cancelled &&  THRightPanState.lastState != .failed {
                return
            }
            ...
            THRightPanState.frameAtStartOfPan = rightContainerView.frame
            THRightPanState.startPointOfPan = panGesture.location(in: view)
            THRightPanState.wasOpenAtStartOfPan =  isRightOpen()
            THRightPanState.wasHiddenAtStartOfPan = isRightHidden()
            
            rightViewController?.beginAppearanceTransition(THRightPanState.wasHiddenAtStartOfPan, animated: true)
            
            addShadowToView(targetContainerView: rightContainerView)
            setOpenWindowLevel()
        case UIGestureRecognizer.State.changed:
            if THRightPanState.lastState != .began && THRightPanState.lastState != .changed {
                return
            }
            
            let translation: CGPoint = panGesture.translation(in: panGesture.view!)
            rightContainerView.frame = applyRightTranslation(translation: translation, toFrame: THRightPanState.frameAtStartOfPan)
            applyRightOpacity()
            applyRightContentViewScale()
            
        case UIGestureRecognizer.State.ended, UIGestureRecognizer.State.cancelled:
            if THRightPanState.lastState != .changed {
                return
            }
            ...
            if panInfo.action == .Open {
                if !THRightPanState.wasHiddenAtStartOfPan {
        ...
        THRightPanState.lastState = panGesture.state
    }
    ...
    public func openLeftWithVelocity(velocity: CGFloat) {
        ...
        var duration: TimeInterval = Double(JKSlideMenuOptions.animationDuration)
        if velocity != 0.0 {
        ...
        UIView.animate(withDuration: duration, delay: 0.0, options: UIView.AnimationOptions.curveEaseInOut, animations: { [weak self]() -> Void in
            if let strongSelf = self {
                ...
                strongSelf.opacityView.layer.opacity = Float(JKSlideMenuOptions.contentViewOpacity)
                
                JKSlideMenuOptions.contentViewDrag == true ? (strongSelf.mainContainerView.transform = CGAffineTransform(translationX: JKSlideMenuOptions.leftViewWidth, y: 0)) : (strongSelf.mainContainerView.transform = CGAffineTransform(scaleX: JKSlideMenuOptions.contentViewScale, y: JKSlideMenuOptions.contentViewScale))
    ...
    public func openRightWithVelocity(velocity: CGFloat) {
        ...
        //    CGFloat finalXOrigin = JKSlideMenuOptions.rightViewOverlapWidth;
        ...
        UIView.animate(withDuration: duration, delay: 0.0, options: UIView.AnimationOptions.curveEaseInOut, animations: { [weak self]() -> Void in
            if let strongSelf = self {
                ...
                JKSlideMenuOptions.contentViewDrag == true ? (strongSelf.mainContainerView.transform = CGAffineTransform(translationX: -JKSlideMenuOptions.rightViewWidth, y: 0)) : (strongSelf.mainContainerView.transform = CGAffineTransform(scaleX: JKSlideMenuOptions.contentViewScale, y: JKSlideMenuOptions.contentViewScale))
            }
        }) { [weak self](Bool) -> Void in
            if let strongSelf = self {
                strongSelf.disableContentInteraction()
    ...
    public func isLeftOpen() -> Bool {leftViewController != nil && leftContainerView.frame.origin.x == 0.0}
    public func isLeftHidden() -> Bool {leftContainerView.frame.origin.x <= leftMinOrigin()}
    ...
    public func isRightOpen() -> Bool {rightViewController != nil && round(rightContainerView.frame.origin.x) == round(view.bounds.width - rightContainerView.frame.size.width)}
    public func isRightHidden() -> Bool {rightContainerView.frame.origin.x >= view.bounds.width}
    ...
    public func changeMainViewController(mainViewController: UIViewController,  close: Bool) {
        ...
        self.fadeAnimationWithView(view: self.mainViewController!.view)
    }
    
    public func changeMainViewController(mainViewController: UIViewController,  close: Bool, pushedVC: UIViewController) {
        
        removeViewController(viewController: self.mainViewController)
        self.mainViewController = mainViewController
//        addAnotherChild(targetView: mainContainerView, targetViewController: pushedVC)
        setUpViewController(targetView: mainContainerView, targetViewController: mainViewController)
        if (close) {
            closeLeft()
            closeRight()
        ...
        signInNavigationController.pushViewController(pushedVC, animated: true)
        isFromMenu = true
//        changeMainViewController(mainViewController: pushedVC, close: true)
    }
    
    public func fadeAnimationWithView(view:UIView){
        view.alpha = 0
    ...
    public func changeLeftViewWidth(width: CGFloat) {
        ...
        JKSlideMenuOptions.leftViewWidth = width
        var leftFrame: CGRect = view.bounds
    ...
    public func changeRightViewWidth(width: CGFloat) {
        ...
        JKSlideMenuOptions.rightBezelWidth = width
        var rightFrame: CGRect = view.bounds
    ...
    private func leftMinOrigin() -> CGFloat {
        return  -JKSlideMenuOptions.leftViewWidth
    }
    ...
    private func panLeftResultInfoForVelocity(velocity: CGPoint) -> PanInfo {
        ...
        let pointOfNoReturn: CGFloat = CGFloat(floor(leftMinOrigin())) + JKSlideMenuOptions.pointOfNoReturnWidth
    ...
    private func panRightResultInfoForVelocity(velocity: CGPoint) -> PanInfo {
        ...
        let thresholdVelocity: CGFloat = -1000.0
        let pointOfNoReturn: CGFloat = CGFloat(floor(view.bounds.width) - JKSlideMenuOptions.pointOfNoReturnWidth)
    ...
    private func applyLeftOpacity() {
        ...
        let opacity: CGFloat = JKSlideMenuOptions.contentViewOpacity * openedLeftRatio
    ...
    private func applyRightOpacity() {
        ...
        let opacity: CGFloat = JKSlideMenuOptions.contentViewOpacity * openedRightRatio
        opacityView.layer.opacity = Float(opacity)
    }
    
    private func applyLeftContentViewScale() {
        let openedLeftRatio: CGFloat = getOpenedLeftRatio()
        let scale: CGFloat = 1.0 - ((1.0 - JKSlideMenuOptions.contentViewScale) * openedLeftRatio);
        let drag: CGFloat = JKSlideMenuOptions.leftViewWidth + leftContainerView.frame.origin.x
        
        JKSlideMenuOptions.contentViewDrag == true ? (mainContainerView.transform = CGAffineTransform(translationX: drag, y: 0)) : (mainContainerView.transform = CGAffineTransform(scaleX: scale, y: scale))
    }
    
    private func applyRightContentViewScale() {
        let openedRightRatio: CGFloat = getOpenedRightRatio()
        let scale: CGFloat = 1.0 - ((1.0 - JKSlideMenuOptions.contentViewScale) * openedRightRatio)
    ...
    private func addShadowToView(targetContainerView: UIView) {
        ...
        targetContainerView.layer.shadowOffset = JKSlideMenuOptions.shadowOffset
        targetContainerView.layer.shadowOpacity = Float(JKSlideMenuOptions.shadowOpacity)
        targetContainerView.layer.shadowRadius = JKSlideMenuOptions.shadowRadius
    ...
    private func addContentOpacity() {
        opacityView.layer.opacity = Float(JKSlideMenuOptions.contentViewOpacity)
    }
    ...
    private func setOpenWindowLevel() {
        if (JKSlideMenuOptions.hideStatusBar) {
    ...
    private func setUpViewController(targetView: UIView, targetViewController: UIViewController?) {
        if let viewController = targetViewController {
            ...
            let targetframe =  targetView.bounds
            viewController.view.frame = targetframe //targetView.bounds
    ...
    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldRecognizeSimultaneouslyWith otherGestureRecognizer: UIGestureRecognizer) -> Bool {
        return JKSlideMenuOptions.simultaneousGestureRecognizers
    }
    
    private func slideLeftForGestureRecognizer( gesture: UIGestureRecognizer, point:CGPoint) -> Bool{
        return isLeftOpen() || JKSlideMenuOptions.panFromBezel && isLeftPointContainedWithinBezelRect(point: point)
    }
    
    private func isLeftPointContainedWithinBezelRect(point: CGPoint) -> Bool{
        //        if let bezelWidth = JKSlideMenuOptions.leftBezelWidth {
    ...
    private func slideRightViewForGestureRecognizer(gesture: UIGestureRecognizer, withTouchPoint point: CGPoint) -> Bool {
        return isRightOpen() || JKSlideMenuOptions.rightPanFromBezel && isRightPointContainedWithinBezelRect(point: point)
    }
    
    private func isRightPointContainedWithinBezelRect(point: CGPoint) -> Bool {
        //        if let rightBezelWidth = JKSlideMenuOptions.rightBezelWidth {
...
extension UIViewController {
    ...
    public func slideMenuController() -> JKSlideMenuController? {
        var viewController: UIViewController? = self
        while viewController != nil {
            if viewController is JKSlideMenuController {
                return viewController as? JKSlideMenuController
            }
