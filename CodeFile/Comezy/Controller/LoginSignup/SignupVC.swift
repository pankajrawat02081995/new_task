...
class SignupVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        initialLoad()
        self.setRightImageOnPassword()
        self.setRightImageOnCponfirmPassword()
    }
    ...
    @IBAction func back_action(_ sender: Any) {
        if self.txtFullName.text?.isEmpty == true && self.txtLastName.text?.isEmpty == true && self.txtPhoneNumber.text?.isEmpty == true && self.txtEmail.text?.isEmpty == true && self.txtOccupation.text?.isEmpty == true && self.txtCompanyName.text?.isEmpty == true && self.txtPassword.text?.isEmpty == true && self.txtCpassword.text?.isEmpty == true{
            ...
            self.showAlertCustomMessage(title: "Alert", message: "You are few steps away from signed up\n Do you still want to exit the sign up flow?") {
                if isFromDynamicLink == true {
                    isFromDynamicLink = false
                }
                self.navigationController?.popViewController(animated: true)
...
extension SignupVC {
    func initialLoad() {
        ...
        txtAbnNumber.attributedPlaceholder = NSAttributedString(string: "Enter Your ABN", attributes: [NSAttributedString.Key.foregroundColor: UIColor.init(rgb: 9808306)])
        txtBuilderAddress.attributedPlaceholder = NSAttributedString(string: "Enter Your Adddress", attributes: [NSAttributedString.Key.foregroundColor: UIColor.init(rgb: 9808306)])
...
extension SignupVC{
    ...
    func setRightImageOnCponfirmPassword(){
        cButton.setImage(UIImage(systemName: "eye.fill"), for: .normal)
        cButton.setImage(UIImage(systemName: "eye.slash.fill"), for: .selected)
        cButton.imageEdgeInsets = UIEdgeInsets(top: 0, left: -12, bottom: 0, right: 0)
        cButton.addTarget(self, action: #selector(toggleCPasswordView), for: .touchUpInside)
        txtCpassword.rightView = cButton
        txtCpassword.rightViewMode = .always
        cButton.alpha = 1.0
        cButton.tintColor = .white
    }
    
    @objc func toggleCPasswordView(_ sender: Any) {
        txtCpassword.isSecureTextEntry.toggle()
        cButton.isSelected.toggle()
    }
