//
//  AddWorkHealthSafetyPlanVC.swift
//  Comezy
//
...
class AddWorkHealthSafetyPlanVC: UIViewController {
    ...
    @IBAction func btnDone_action(_ sender: Any) {
        ...
        self.objSafety.addSafety(name: txtName.text ?? "", description: txtDescription.text ?? "", file: arrayOfAddedVariationsURL, project: ProjectId ?? 0, type: "work_health_and_safety_plan") { success, eror, type in
            if success {
                self.navigationController?.popViewController(animated: true)

                self.showToast(message: SuccessMessage.kWorkHealthSafetyAdded, font: .boldSystemFont(ofSize: 14.0))
...
//MARK: - Extension AddWorkHealthSafetyPlanVC For Image Picker Controller
extension AddWorkHealthSafetyPlanVC: UIImagePickerControllerDelegate,UINavigationControllerDelegate,  PhotoEditorDelegate{
...
extension AddWorkHealthSafetyPlanVC: UIDocumentPickerDelegate{
    ...
    func uploadFile(urls:[URL]){
        if arrayOfAddedVariations.count < 10 {
            ...
            print("urls[0]", urls[0])
            arrayPath = urls[0]
            AWSS3Manager.shared.uploadImageFile(fileUrl: urls[0], fileName:"\(AWSFileDirectory.PUBLIC)\(AWSFileDirectory.WORK_HEALTH_SAFETY_PLAN_FOLDER)\(AWSFileDirectory.WORK_HEALTH_SAFETY_PLAN_FILE)\(arrayPath!.lastPathComponent)") { (progress) in
...
extension AddWorkHealthSafetyPlanVC: UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
