...
class SafetyWorkMethodVC: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        NotificationsViewModel.shared.clearBadgeCount(projectId: ProjectId ?? 0, module: "safe_work_method_statement") { success, resp, errorMsg in
            if success {
    ...
    func getDocsListDetails() {
        self.objVM.getList(size: "1000", page: 1, project_id: ProjectId ?? 0, type: "safe_work_method_statement" ) { (success, resp, error) in
                if success {
    ...
    @IBAction func btnAddROI_action(_ sender: Any) {
        let vc = ScreenManager.getController(storyboard: .safety, controller: AddSafetyWorkMethodVC()) as! AddSafetyWorkMethodVC
...
extension SafetyWorkMethodVC: UITableViewDelegate, UITableViewDataSource {
