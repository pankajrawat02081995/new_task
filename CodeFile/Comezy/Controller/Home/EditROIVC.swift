...
class EditROIVC: UIViewController {

    var variationId: Int?
    let objAddROIVM = AddROIViewModel.shared
    let objPeopleListViewModel = PeopleListViewModel.shared
    var objROIDetailData: ROIDetailModel?
    var objEditROIModelVM = EditROIViewModel()
    ...
    func loadInformation() {
        ...
        txtVariationSummary.text = ref.infoNeeded
    ...
    @IBAction func btnUpdate_action(_ sender: Any) {
        ...
        vc.show()
        vc.confirmHeadingLabel.text = "Update Request of Information"
        vc.confirmDescriptionLabel.text = "Do you want to update the Request of Information?"
        vc.callback = {
            let ref = self.objEditROIModelVM
            ref.updateROI(name: self.txtTitle.text ?? "", infoNeeded: self.txtVariationSummary.text ?? "", file: self.arrayOfAddedVariationsURL, roiId: self.variationId ?? 0) { success, eror, type in
                if success {
                    self.navigationController?.popViewController(animated: true)
                    self.showToast(message: SuccessMessage.kRoiAdded, font: .boldSystemFont(ofSize: 14.0))
...
//MARK: - Extension EditROIVC For Image Picker Controller
extension EditROIVC: UIImagePickerControllerDelegate,UINavigationControllerDelegate , PhotoEditorDelegate {
...
extension EditROIVC: UIDocumentPickerDelegate{
    ...
    func uploadFile(urls:[URL]){
        if arrayOfAddedVariations.count < 10 || selectedIndex != nil{
            ...
            print("urls[0]", urls[0])
            arrayPath = urls[0]
            AWSS3Manager.shared.uploadImageFile(fileUrl: urls[0], fileName: "\(AWSFileDirectory.PUBLIC)\(AWSFileDirectory.REQUEST_OF_INFORMATION_FOLDER)\(AWSFileDirectory.REQUEST_OF_INFORMATION_FILE)\(arrayPath!.lastPathComponent)") { (progress) in
...
extension EditROIVC: UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
