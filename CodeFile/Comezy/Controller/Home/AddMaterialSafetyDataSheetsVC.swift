//
//  AddMaterialSafetyDataSheetsVC.swift
//  Comezy
//
...
class AddMaterialSafetyDataSheetsVC: UIViewController {
    
    
    let objSafety = SafetyListViewModel.shared
    ...
    @IBAction func btnDone_action(_ sender: Any) {
        ...
        self.objSafety.addSafety(name: txtName.text ?? "", description: txtDescription.text ?? "", file: arrayOfAddedVariationsURL, project: ProjectId ?? 0, type: "material_safety_data_sheets") { success, eror, type in
            if success {
                self.navigationController?.popViewController(animated: true)
                
                self.showToast(message: SuccessMessage.kSafetyAdded, font: .boldSystemFont(ofSize: 14.0))
...
//MARK: - Extension AddMaterialSafetyDataSheetsVC For Image Picker Controller
extension AddMaterialSafetyDataSheetsVC: UIImagePickerControllerDelegate,UINavigationControllerDelegate, PhotoEditorDelegate{
...
extension AddMaterialSafetyDataSheetsVC: UIDocumentPickerDelegate{
    ...
    func uploadFile(urls:[URL]){
        if arrayOfAddedVariations.count < 10 {
            ...
            print("urls[0]", urls[0])
            arrayPath = urls[0]
            AWSS3Manager.shared.uploadImageFile(fileUrl: urls[0], fileName:"\(AWSFileDirectory.PUBLIC)\(AWSFileDirectory.MATERIAL_SAFETY_DATA_SHEET_FOLDER)\(AWSFileDirectory.MATERIAL_SAFETY_DATA_SHEET_FILE)\(arrayPath!.lastPathComponent)") { (progress) in
...
extension AddMaterialSafetyDataSheetsVC: UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
