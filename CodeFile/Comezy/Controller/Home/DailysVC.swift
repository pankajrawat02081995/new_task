...
//                self.showToast(message: "Schedule work is in progress")
            ...
            if indexPath.row == 4 {
                ...
                let vc = ScreenManager.getController(storyboard: .main, controller: AccountingVC()) as! AccountingVC
                vc.projectId = ProjectId
                self.navigationController?.pushViewController(vc, animated: true)
//                let vc = ScreenManager.getController(storyboard: .main, controller: AccountingVC()) as! AccountingVC
//                //vc.ProjectId = ProjectId
...
//                self.navigationController?.pushViewController(vc, animated: true)
                ...
                let vc = ScreenManager.getController(storyboard: .main, controller: DailyAccountingVC()) as! DailyAccountingVC
                vc.projectId = ProjectId ?? 0
                self.navigationController?.pushViewController(vc, animated: true)
            }
        }
    }
