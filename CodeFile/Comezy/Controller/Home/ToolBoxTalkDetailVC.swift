...
class ToolBoxTalkDetailVC: UIViewController {
    var tbID: Int?
    let objToolBoxDetailVM = ToolBoxTalkDetailViewModel()
    let objDeleteToolBoxVM = DeleteVariationViewModel()
    var objToolBoxDetailModel: ToolBoxTalkDetailModel?
...
//    let objVariationResponseData: BaseResponse<VariationResponseDataModel>? = nil
    ...
    @IBOutlet weak var lblTopicOfDiscussion: UILabel!
    @IBOutlet weak var lblRemedies: UILabel!
    ...
    func initialLoad() {
        ...
        objToolBoxDetailVM.getToolBoxDetail(tbId: tbID ?? 0 ) { success, response, errorMsg in
           
            if success {
                self.objToolBoxDetailModel = response
                self.variationDocsArray = response?.file
        ...
        objCommentListViewModel.getCommentList(module: "toolbox_id", module_id: tbID ?? 34, size: 1000, page: 1) { success, comments, msg in
            if success {
    ...
    func updateUI() {
        ...
        let ref = objToolBoxDetailModel
        variationTitleLabel.text = ref?.name
        lblTopicOfDiscussion.text = ref?.topicOfDiscussion
        lblRemedies.text = ref?.remedies
        ...
        DispatchQueue.main.async {
            self.tableViewHeightConstraint.constant = self.tableView.contentSize.height + CGFloat((15 * (self.objToolBoxDetailModel?.receiver.count ?? 0)))
        }
    ...
    */
    ...
    @IBAction func btnEdit_action(_ sender: Any) {
        let vc = ScreenManager.getController(storyboard: .general, controller: EditToolBoxVC()) as! EditToolBoxVC
        vc.objROIDetailData = self.objToolBoxDetailModel
        vc.variationId = self.tbID
    ...
    @IBAction func btnDelete_action(_ sender: Any) {
        showAppAlert(title: "Delete Variation", message: "Do you want to delete this variation?") {
            self.objDeleteToolBoxVM.deleteVariation(variation_id: self.tbID ?? 0) { success, response, msg in
                if success {
                    DispatchQueue.main.async {
    ...
    @IBAction func btnAddComment_action(_ sender: Any) {
        ...
        vc.callback = {
            ...
            if vc.commentTextField.text == "Write a comment here" || vc.commentTextField.text == "" || vc.commentTextField.text == "Cannot post an empty comment" {
                ...
                self.objCommentListViewModel.getAddComment(controller: self, module: "toolbox", comment: vc.commentTextField.text!, moduleId: self.tbID ?? 0) { success, addTaskList, errorMsg in
                    
                    if success {
                        self.showToast(message: "Comment added successfully", font: .boldSystemFont(ofSize: 14.0))

                        self.objCommentListViewModel.getCommentList(module: "toolbox_id", module_id: self.tbID ?? 0, size: 1000, page: 1) { success, allCommments, msg in
                            if success {
...
extension ToolBoxTalkDetailVC: UITableViewDelegate, UITableViewDataSource {
    
    func numberOfSections(in tableView: UITableView) -> Int {
        if tableView == variationCommentTableView {
            return 1
        } else {
            return objToolBoxDetailModel?.receiver.count ?? 0
        }
            ...
            if ref?.receiver[indexPath.section].id as? Int == loggedInUserId {
                if self.variationResponseStatus == "pending" {
                    ...
                    cell.xMarkCallback = {
                        ...
                        self.showAppAlert(title: "Reject Tool Box", message: "Do you want to reject the Tool Box") {
                            self.objToolBoxDetailVM.getResponse(tbId: self.tbID ?? 0, accepted: "reject") { success, variationDetailResp, errorMsg in
                                if success {
                                    self.showToast(message: "Tool Box rejected succcessfully")
                                    print("Tool Box rejected succcessfully")
                                    self.objToolBoxDetailVM.getToolBoxDetail(tbId: self.tbID ?? 0 ) { success, response, errorMsg in
    ...
    //                            }
                    ...
                    cell.checkMarkCallback = {
                        ...
                        self.showAppAlert(title: "Accept Tool Box", message: "Do you want to accept the Tool Box?") {
                            self.objToolBoxDetailVM.getResponse(tbId: self.tbID ?? 0, accepted: "accept") { success, variationDetailResp, errorMsg in
                                if success {
                                    self.objToolBoxDetailVM.getToolBoxDetail(tbId: self.tbID ?? 0 ) { success, response, errorMsg in
                                        self.showToast(message: "Tool Box accepted succcessfully")
                                        if success {
                                            self.objToolBoxDetailModel = response
                                            if let receivers = response?.receiver {
                                                for i in receivers {
                                    ...
                                    print("Tool Box accepted succcessfully")
                                }
                                else {
                                    self.showAlert(message: errorMsg)
                                }
...
extension ToolBoxTalkDetailVC: UICollectionViewDelegate, UICollectionViewDataSource {
