...
class HomeVC: UIViewController{
    ...
    override func viewDidLoad() {
        ...
        self.lblNotificationsCount.layer.cornerRadius = self.lblNotificationsCount.frame.size.height / 2
        ...
        if(kUserData?.user_type == UserType.kOwner){
            objSignupViewModel.profileDetail(){ [self] success, response, message in
                if success {
                    ...
                    if let trialDate = response?.trialEnded {
                        self.trialEndDate = trialDate
                        if strValue > trialEndDate {
                            print("has plan")
                        } else {
                            print("no plan")
                        }
                            ...
                            .addSnapshotListener { (snapshot,error) in
                                if let error = error {
                                    ...
                                    if(a as! Int != 0 ){
                                        ...
                                        self.btnSubsribeAction.setTitle("", for: .normal)
                                        self.btnSubsribeAction.isEnabled = true
                                        self.btnSubsribeAction.isHidden = false
                                    }
                                }
                            }
    ...
    func getNoti(){
        objAllNotifVM.getAllNotificationsCount { success, resp, errorMsg in
            if success {
                self.lblNotificationsCount.text = "\(resp?.notificationCount ?? 0)"
                if resp?.notificationCount ?? 0 == 0{
                    self.lblNotificationsCount.isHidden = true
                }else{
                    self.lblNotificationsCount.isHidden = false
                }
            }
        }
    }
    ...
    @IBAction func addProject_action(_ sender: Any) {
        ...
        //        if self.trialEndDate > self.todayDate {
...
extension HomeVC : UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //        let dateFormatter = DateFormatter()
        //        dateFormatter.dateFormat = "yyyy-MM-dd"
        //        let todayDateValue = dateFormatter.date(from: self.todayDate)
        //        let trialDateValue = dateFormatter.date(from: self.trialEndDate)
        //        print(todayDateValue ?? Date(), trialDateValue ?? Date())
        //        if trialDateValue ?? Date() < todayDateValue ?? Date() {
        //            print("ha")
        //        } else {
        //            print("na")
        //        }
