...
class EditPlanVC: UIViewController{
 ...
 let objSafetyViewModel = SafetyListViewModel.shared
    var objSafetyDetail: PlansResult?
    ...
    @IBAction func btnUpdate_action(_ sender: Any) {
        ...
        vc.show()
        vc.confirmHeadingLabel.text = "Update Plan?"
        vc.confirmDescriptionLabel.text = "Do you want to update the Plan?"
        vc.callback = {
            let ref = self.objPlansListViewModel
            
            
            ref.updatePlan(name: self.txtTitle.text ?? "" , description: self.txtDescription.text ?? "", file: self.arrayOfAddedFilesUrl, planId: self.safetyId ?? 0) { success, eror, response in
                if success {
                    self.showToast(message: SuccessMessage.kPlanUpdated, font: .boldSystemFont(ofSize: 14.0))
...
extension EditPlanVC: UIImagePickerControllerDelegate,UINavigationControllerDelegate, PhotoEditorDelegate {
...
extension EditPlanVC: UIDocumentPickerDelegate{
...
extension EditPlanVC: UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
