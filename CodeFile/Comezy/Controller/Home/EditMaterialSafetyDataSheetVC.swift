//
//  EditMaterialSafetyDataSheetVC.swift
//  Comezy
//
...
class EditMaterialSafetyDataSheetVC: UIViewController {
    ...
    var arrayPath : URL?
    var previousRef: MaterialSafetyDataDetailVC?
    ...
    @IBAction func btnUpdate_action(_ sender: Any) {
        ...
        vc.show()
        vc.confirmHeadingLabel.text = "Update Material Safety Method?"
        vc.confirmDescriptionLabel.text = "Do you want to update the Material safety data?"
        vc.callback = {
            ...
            ref.updateSafety(name: self.txtTitle.text ?? "" , description: self.txtDescription.text ?? "", file: self.arrayOfAddedFilesUrl, safetyId: self.safetyId ?? 0) { success, eror, response in
                if success {
                    self.showToast(message: SuccessMessage.kSafetyUpdated, font: .boldSystemFont(ofSize: 14.0))
...
//MARK: - Extension EditMaterialSafetyDataSheetVC For Image Picker Controller
extension EditMaterialSafetyDataSheetVC: UIImagePickerControllerDelegate,UINavigationControllerDelegate , PhotoEditorDelegate{
...
extension EditMaterialSafetyDataSheetVC: UIDocumentPickerDelegate{
...
extension EditMaterialSafetyDataSheetVC: UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
