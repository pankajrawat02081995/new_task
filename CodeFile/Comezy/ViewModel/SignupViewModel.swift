    ...
    func getsignupDetails(signupType: String,controller: UIViewController, first_name:String,last_name:String, user_type:String,email:String,password: String, cpassword: String, phone: String, facebook_token: String, apple_token: String, google_token: String, occupation: Int, company: String, signature: String, abnNumber: String, builderAddress: String, completionHandler:@escaping (_ sucess:Bool,_ eror:String?,_ type: String?) -> Void) {
        if signupType == "social" {
            ...
            if (first_name.trimmingCharacters(in: .whitespaces).isEmpty) {
                ...
                serviceSignup(controller: controller, first_name: first_name, last_name: last_name, user_type: user_type, email: email, password: password, phone: phone, facebook_token: facebook_token, apple_token: apple_token, google_token: google_token, occupation: occupation, company: company, signature: signature, abn: abnNumber, builderAddress: builderAddress) { (status, userDetail, error, mytype) in
                if status {
            ...
            if (first_name.trimmingCharacters(in: .whitespaces).isEmpty) {
                completionHandler(false,FieldValidation.kFirstNameEmpty, "")
            } else if first_name.count > 16 {
                completionHandler(false,FieldValidation.kFirstName, "")
            } else if (last_name.trimmingCharacters(in: .whitespaces).isEmpty){
                completionHandler(false,FieldValidation.kLastNameEmpty, "")
            } else if last_name.count > 16 {
                completionHandler(false,FieldValidation.kLastName, "")
            } else if (phone.trimmingCharacters(in: .whitespaces).isEmpty){
                completionHandler(false,FieldValidation.kPhoneEmpty, "")
            } else if phone.count < 10 {
                completionHandler(false,FieldValidation.kValidPhone, "")
            } else if (email.trimmingCharacters(in: .whitespaces).isEmpty){
                completionHandler(false,FieldValidation.kEmailEmpty, "")
            }else if !email.isvalidEmail {
                completionHandler(false,FieldValidation.kValidEmail , "")
                ...
                if (builderAddress.isEmpty) {
                    completionHandler(false,FieldValidation.kBuilderAddressEmpty, "")
                } else if (abnNumber.isEmpty) {
                    completionHandler(false,FieldValidation.kAbnEmpty, "")
                } else if (abnNumber.count != 11) {
                    completionHandler(false,FieldValidation.kInvalidAbnEmpty, "")
                } else {
                    serviceSignup(controller: controller, first_name: first_name, last_name: last_name, user_type: user_type, email: email, password: password, phone: phone, facebook_token: facebook_token, apple_token: apple_token, google_token: google_token, occupation: occupation, company: company, signature: signature, abn: abnNumber, builderAddress: builderAddress) { (status, userDetail, error, mytype) in
                        if status {
...
//                completionHandler(false,FieldValidation.kvalidPassword , "")
//            }
            else {
                serviceSignup(controller: controller, first_name: first_name, last_name: last_name, user_type: user_type, email: email, password: password, phone: phone, facebook_token: facebook_token, apple_token: apple_token, google_token: google_token, occupation: occupation, company: company, signature: signature, abn: abnNumber, builderAddress: builderAddress) { (status, userDetail, error, mytype) in
                    if status {
        ...
        if google_token != "" {
         ...
         //parameters = ["first_name":first_name,"last_name":last_name, "user_type":user_type,"email":email, "google_token":google_token]
            ...
            //parameters = ["first_name":first_name,"last_name":last_name, "user_type":user_type,"email":email, "apple_token":apple_token]
               ...
               parameters = ["first_name":first_name,"last_name":last_name, "user_type":user_type,"email":email, "password":password,"phone":phone, "facebook_token":facebook_token,"apple_token":apple_token,"google_token":google_token, "occupation":occupation, "company":company, "signature":signature, "ABN": abn, "address": builderAddress]
           }
        DispatchQueue.main.async {
            APIManager.shared.requestWithoutHeaders(url: API.registerApi, method: .post,parameters:parameters ,completionCallback: { (_ ) in
                ...
                if let json = json {
                    do {
                        ...
                        if profile.code == 200 {
                            ...
                            let fName = profileWithId.data?.firstName
                            let lName = profileWithId.data?.lastName
                            let fullName = "\(fName ?? "") \(lName ?? "")"
                            defaults.set(fullName, forKey: "FullName")
                            defaults.set(profileWithId.data?.email, forKey: "email")
                           // defaults.set(profileWithId.data, forKey: "")
