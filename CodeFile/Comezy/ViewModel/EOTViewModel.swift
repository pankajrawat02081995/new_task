//
//  ExtensionOfTimeViewModel.swift
//  Comezy
//
...
class EOTViewModel: NSObject {
    func getList(size: String, page: Int, project_id: Int, completionHandler: @escaping(_ success: Bool, _ resp: EOTListModel?, _ errorMsg: String?) -> Void){
      
        DispatchQueue.main.async{
            var url = API.getEOTList
            ...
            APIManager.shared.request(url: url, method: .get, parameters:nil ,completionCallback: { (_ ) in
                ...
                if let json = json {
                    do {
                        ...
                        let result = try JSONDecoder().decode(BaseResponse<EOTListModel>.self, from: jsonData)
