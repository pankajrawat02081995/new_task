...
class AddROIVC: UIViewController {

 
    let objAddROIViewModel = AddROIViewModel.shared
...
//        }
    ...
    @IBAction func btnDone_action(_ sender: Any) {
        ...
        self.objAddROIViewModel.addROI(controller: self, name: self.txtName.text ?? "" , sender: self.selectedFromPersonId , infoNeeded: self.txtInfoNeeded.text ?? "" , file: self.arrayOfAddedVariationsURL, project: self.ProjectId ?? 0, receiver: receiverIdArray) { success, eror, type in
            if success {
                self.navigationController?.popViewController(animated: true)

                self.showToast(message: "Request of Information added successfully", font: .boldSystemFont(ofSize: 14.0))
...
extension AddROIVC: UIImagePickerControllerDelegate,UINavigationControllerDelegate, PhotoEditorDelegate {
...
extension AddROIVC: UIDocumentPickerDelegate{
...
extension AddROIVC: UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
