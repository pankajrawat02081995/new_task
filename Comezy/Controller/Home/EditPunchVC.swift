...
class EditPunchVC: UIViewController {
    ...
    var objEditPunchViewModel = EditPunchViewModel()
    ...
    var recievers: [PunchDetailSender]?
    ...
    @IBOutlet weak var addPunchTableViewHeightConstraint: NSLayoutConstraint!
    ...
    func loadInformation() {
        guard let ref = objPunchDetailData else {return}
        ...
        txtVariationSummary.text = ref.welcomeDescription
        
        for i in ref.checklist {
            checkList.append(i.name)
        }

        
        addPunchTableViewHeightConstraint.constant = CGFloat(130 * checkList.count)
    ...
    @IBAction func btnUpdate_action(_ sender: Any) {
        print(variationId, "@#$!@#$%@#$%@#$%@#$%@#$%@#$!$#!@#$!@#$@#$%@#%^#$%@#$@#$@$#%!@#$%!@#$!@#$!@#$!@#$!@#$!@#$!#@$!@#$!@#$!@#$!@#$!@#$!@#$%@#%@#%^##$%&##$!@#$!@#$!@#$#%^#%^&#^%&#$^%#$%@#$%@#$2013984710293847109238471098537498571-23481724309817350981234709182364109237561028394")
        let vc = ConfirmViewController()
        vc.show()
        vc.confirmHeadingLabel.text = "Update Punch"
        vc.confirmDescriptionLabel.text = "Do you want to update the Punch?"
        vc.callback = {
            let ref = self.objEditPunchViewModel
            ref.updatePunch(name: self.txtTitle.text ?? "", infoNeeded: self.txtVariationSummary.text ?? "", file: self.arrayOfAddedVariationsURL, punchId: self.variationId ?? 0, checklist: self.checkList) { success, eror, type in
                if success {
...
//        }
    ...
    @IBAction func addCheck_action(_ sender: Any) {
        checkList.append("")
        addPunchTableViewHeightConstraint.constant += 130
        addPunchCheckTableView.reloadData()
...
//MARK: - Extension EditPunchVC For Image Picker Controller
extension EditPunchVC: UIImagePickerControllerDelegate,UINavigationControllerDelegate , PhotoEditorDelegate {
...
extension EditPunchVC: UIDocumentPickerDelegate{
...
extension EditPunchVC: UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
...
extension EditPunchVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == addPunchCheckTableView {
            ...
            cell.didEndEditCallback = { text in
                self.checkList[indexPath.section] = text
            }
            cell.punchTextField.text = checkList[indexPath.section]
            cell.cancelCallback = {
                self.checkList.remove(at: indexPath.section)
                self.addPunchTableViewHeightConstraint.constant -= 130
