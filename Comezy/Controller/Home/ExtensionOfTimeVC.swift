...
class ExtensionOfTimeVC: UIViewController {
    
    var loggedInUserOcc: String?
    var ProjectId: Int?
    let objVM = EOTViewModel()
    var myList = [EOTListResult]()
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        obj.clearBadgeCount(projectId: ProjectId ?? 0, module: "eot") { success, resp, errorMsg in
            if success {
    ...
    @IBAction func btnAdd_action(_ sender: Any) {
        let vc = ScreenManager.getController(storyboard: .general, controller: AddEOTVC()) as! AddEOTVC
...
extension ExtensionOfTimeVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.lblPlanDescription.text = item.reasonForDelay
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let vc = ScreenManager.getController(storyboard: .general, controller: EOTDetailVC()) as! EOTDetailVC
        vc.eotId = myList[indexPath.row].id
        let item = myList[indexPath.row]
        self.navigationController?.pushViewController(vc, animated: true)
    }
