...
class AddSafetyWorkMethodVC: UIViewController {
...
//        self.flowLayout.estimatedItemSize = UICollectionViewFlowLayout.automaticSize
    ...
    @IBAction func btnDone_action(_ sender: Any) {
        ...
        self.objSafety.addSafety(name: txtName.text ?? "", description: txtDescription.text ?? "", file: arrayOfAddedVariationsURL, project: ProjectId ?? 0, type: "safe_work_method_statement") { success, eror, type in
            if success {
                self.navigationController?.popViewController(animated: true)

                self.showToast(message: SuccessMessage.kSafetyWorkAdded, font: .boldSystemFont(ofSize: 14.0))
...
extension AddSafetyWorkMethodVC: UIImagePickerControllerDelegate,UINavigationControllerDelegate, PhotoEditorDelegate{
...
extension AddSafetyWorkMethodVC: UIDocumentPickerDelegate{
...
extension AddSafetyWorkMethodVC: UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
