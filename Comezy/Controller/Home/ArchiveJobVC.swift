...
//  Comezy
//
//  Created by amandeepsingh on 19/07/22.
//

import Foundation
import UIKit

class ArchiveJobVC: UIViewController {
...
//        AppDelegate.shared.window?.rootViewController = signInNavigationController
    ...
    func getDocsListDetails() {
        ...
        self.objVM.getProjectListByType(type: "archivelist") { (success, resp, error) in
                if success {
...
extension ArchiveJobVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.btnCallback = {
            self.showAppAlert(title: "Archive Project", message: "Do you want to archive this project?") {
                self.objVM.changeProjectStatus(type: "archive", projectId: item.id ?? 0) { status, projectList, errorMsg in
                    if status {
                        self.showToast(message: "Project archived successfully")
