...
class LeaveJobVC: UIViewController {
    
    
    var loggedInUserOcc: String?
    var ProjectId: Int?
    let objVM = ProjectStatusViewModel()
    var myList = [LeaveJobResult]()
...
//        AppDelegate.shared.window?.rootViewController = signInNavigationController
    ...
    func getDocsListDetails() {
        ...
        self.objVM.leaveProjectList { (success, resp, error) in
                if success {
...
extension LeaveJobVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.lblName.text = item.project[0].name
        print("Created Date ->", item.project[0].createdDate)
        print("project Description  ->", item.project[0].projectDescription)
        let dateFormatter = DateFormatter()
        let tempLocale = dateFormatter.locale
        dateFormatter.locale = Locale(identifier: "en_US_POSIX") // set locale to reliable US_POSIX
        dateFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
        let date = dateFormatter.date(from: item.project[0].createdDate)
        ...
        cell.lblSummary.text = item.project[0].projectDescription
...
//        }
        ...
        if item.project[0].status == "in_progress" {
            cell.lblStatus.textColor = .systemYellow
            cell.lblStatus.text = "In Progress"
        } else if item.project[0].status == "completed" {
            cell.lblStatus.textColor = .systemGreen
            cell.lblStatus.text = "Completed"
        } else if item.project[0].status == "paused" {
            cell.lblStatus.textColor = .systemRed
            cell.lblStatus.text = "Paused"
        }
        
        cell.btnChangeStatus.setTitle("Leave", for: .normal)
        cell.btnChangeStatus.backgroundColor = .systemRed
        cell.btnCallback = {
            self.showAppAlert(title: "Leave Project", message: "Do you want to leave this project?") {
                self.objVM.changeProjectStatus(type: "leavejob", projectId: item.project[0].id) { status, projectList, errorMsg in
                    if status {
                        self.showToast(message: "Project left successfully")
                        self.getDocsListDetails()
                    } else {
                        print("Message, ->",errorMsg)
                        self.showToast(message: errorMsg ?? "Error occured please try again later." )
                    }
                }
            }
        }
