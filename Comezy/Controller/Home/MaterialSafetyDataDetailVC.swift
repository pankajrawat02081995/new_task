//
//  MaterialSafetyDataDetailVC.swift
//  Comezy
//
...
class MaterialSafetyDataDetailVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        initialLoad()
        if didEdit == true {
            SafetyListViewModel.shared.getList(size: "1000", page: 1, project_id: projectId ?? 0 , type: "material_safety_data_sheets") { success, resp, errorMsg in
    ...
    @IBAction func btnEdit_action(_ sender: Any) {
        let vc = ScreenManager.getController(storyboard: .safety, controller: EditMaterialSafetyDataSheetVC()) as! EditMaterialSafetyDataSheetVC
    ...
    @IBAction func btnDelete_action(_ sender: Any) {
        self.showAppAlert(title: "Delete Material Safety", message: "Do you want to delete this material safety ") {
            
            SafetyListViewModel.shared.deleteSafety(safetyId: self.safetyId ?? 0) { success, response, errorMsg in
                if success {
                    self.navigationController?.popViewController(animated: true)
                    self.showToast(message: "Material Safety deleted successfully")
                }
            }
...
extension MaterialSafetyDataDetailVC: UICollectionViewDelegate, UICollectionViewDataSource {
