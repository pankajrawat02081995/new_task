...
class AddToolBoxVC: UIViewController {
    
       let objAddToolBoxViewModel = AddToolBoxViewModel()
    ...
    var imageName: String?
       ...
       @IBOutlet weak var txtTopicOfDiscussion: UITextView!
...
//           self.present(documentPicker, animated: true)
       ...
       @IBAction func btnDone_action(_ sender: Any) {
           ...
           self.objAddToolBoxViewModel.addToolBox(name: self.txtName.text ?? "" , sender: self.selectedFromPersonId ?? 0 , topicOfDiscussion: txtTopicOfDiscussion.text ?? "", remedies: txtRemedies.text ?? "", file: self.arrayOfAddedVariationsURL, project: self.ProjectId ?? 0, receiver: receiverIdArray) { success, eror, type in
               if success {
                   self.navigationController?.popViewController(animated: true)

                   self.showToast(message: "Tool box added successfully", font: .boldSystemFont(ofSize: 14.0))
...
//MARK: - Extension AddToolBoxVC For Image Picker Controller
extension AddToolBoxVC: UIImagePickerControllerDelegate,UINavigationControllerDelegate, PhotoEditorDelegate {
...
extension AddToolBoxVC: UIDocumentPickerDelegate{
...
extension AddToolBoxVC: UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
