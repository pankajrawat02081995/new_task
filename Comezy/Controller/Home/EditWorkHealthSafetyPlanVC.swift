//
//  EditWorkHealthSafetyPlanVC.swift
//  Comezy
//
...
class EditWorkHealthSafetyPlanVC: UIViewController {
    ...
    var arrayPath : URL?
    var previousRef: WorkHealthSafetyPlanDetailVC?
    ...
    @IBAction func btnUpdate_action(_ sender: Any) {
        ...
        vc.show()
        vc.confirmHeadingLabel.text = "Update Work health safety plan?"
        vc.confirmDescriptionLabel.text = "Do you want to update the Work health safety plan?"
        vc.callback = {
            ...
            ref.updateSafety(name: self.txtTitle.text ?? "" , description: self.txtDescription.text ?? "", file: self.arrayOfAddedFilesUrl, safetyId: self.safetyId ?? 0) { success, eror, response in
                if success {
                    self.showToast(message: SuccessMessage.kWorkHealthSafetyUpdated, font: .boldSystemFont(ofSize: 14.0))
...
extension EditWorkHealthSafetyPlanVC: UIImagePickerControllerDelegate,UINavigationControllerDelegate,  PhotoEditorDelegate{
...
extension EditWorkHealthSafetyPlanVC: UIDocumentPickerDelegate{
...
extension EditWorkHealthSafetyPlanVC: UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
